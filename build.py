...
and on demand whenever Matthew's Apps Script dispatches it. For each program
(dc, fpb, pom, go) it fetches the latest data JSON from Matthew's Apps Script
endpoint and injects it into the matching template. If the fetch fails or there
is no data yet, the existing page is left untouched.
"""
